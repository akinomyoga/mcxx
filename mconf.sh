...
## 関数 mconf/test/compiles-stdin.impl options
##   @var[in] mconf_cxx
...
##   @fd[out] 16
##     ログファイルへの出力
function mconf/test/compiles-stdin.impl {
  mconf/log/print-title "$mconf_log_title"
  "$mconf_cxx" -c -xc++ - -o tmp.o "${mconf_cxx_options[@]}"
  local ext=$?
  rm -f tmp.o tmp.obj
  return "$ext"
} 1>&6 2>&6
...
## @env[in] mconf_flags
## @env[in] mconf_cache_name
## @env[in] mconf_log_title
## @env[in] mconf_msg_title
## @env[in] mconf_msg_ok
## @env[in] mconf_msg_ng
## @env[in] mconf_cmd_code
function mconf/test/compiles-stdin {
  ...
  if mconf/test/compiles-stdin.impl <<< "$code"; then
    result=1
